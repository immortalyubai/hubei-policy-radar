...
- 3 个已核验湖北公众号的账号级扫描：湖北发布、湖北科技、武汉科技创新；每 2 小时拉取公开文章元数据。
...
- 公众号账号级扫描由本机私有服务完成，管理员本人扫码；政策雷达代码、公开快照和部署平台都不接触或保存微信登录凭证。
...
公众号账号级扫描由本机私有服务完成，不能由 GitHub 托管 Runner 访问。电脑休眠、程序停止或约四天登录到期时，该部分会暂停并要求重新扫码；政府官网扫描继续运行。公开网站展示最近一次安全状态快照，不直接连接本机登录会话。
...
1. 将本机试点迁移到可持续运行的私有主机，并加入登录到期提醒。
